...
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/dashicons.css?v='.@$revision);
$document->addStyleSheet($base . '/components/com_layer_slider/assets/css/wp_specs.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/global.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/admin.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/admin_new.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/layerslider.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/css/layerslider.transitiongallery.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/js/minicolors/jquery.minicolors.css?v='.@$revision);

if($GLOBALS['j25']) {
  $document->addStyleSheet($base . '/components/com_layer_slider/assets/css/layer_slider_25.css?v='.@$revision);
...
$document->addScript($base . '/components/com_layer_slider/assets/js/jquery-ui-1.10.4.custom.min.js');
$document->addScript($base . '/components/com_layer_slider/assets/js/wp_specs.js?v='.@$revision);

$document->addScript($root . '/components/com_layer_slider/base/static/js/admin.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/js/builder.js?v='.@$revision);
...
$document->addScript($root . '/components/com_layer_slider/base/static/js/layerslider.transition.gallery.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/js/minicolors/jquery.minicolors.js?v='.@$revision);

// 3rd-party: CodeMirror
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/codemirror/lib/codemirror.css?v='.@$revision);
$document->addStyleSheet($root . '/components/com_layer_slider/base/static/codemirror/theme/solarized.mod.css?v='.@$revision);

$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/lib/codemirror.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/mode/css/css.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/mode/javascript/javascript.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/addon/fold/foldcode.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/addon/fold/foldgutter.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/addon/fold/brace-fold.js?v='.@$revision);
$document->addScript($root . '/components/com_layer_slider/base/static/codemirror/addon/selection/active-line.js?v='.@$revision);
...
function jols_update_option($option, $value){
	$db = JFactory::getDbo();
...
function jols_nonce_url( $actionurl, $action = -1, $name = '_wpnonce' ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return $actionurl;
}

function jols_nonce_field( $action = -1, $name = "_wpnonce", $referer = true , $echo = true ) {
...
function jols_n( $single, $plural, $number, $domain = 'default' ) {
	return $number > 1 ? $plural : $single;
}

function jols_human_time_diff( $from, $to = '' ) {
	if ( empty( $to ) )
		$to = time();
	...
	if ( $diff < HOUR_IN_SECONDS ) {
		...
		$since = sprintf( jols_n( '%s min', '%s mins', $mins ), $mins );
	} elseif ( $diff < DAY_IN_SECONDS && $diff >= HOUR_IN_SECONDS ) {
		$hours = round( $diff / HOUR_IN_SECONDS );
		if ( $hours <= 1 )
			$hours = 1;
		$since = sprintf( jols_n( '%s hour', '%s hours', $hours ), $hours );
	} elseif ( $diff < WEEK_IN_SECONDS && $diff >= DAY_IN_SECONDS ) {
		$days = round( $diff / DAY_IN_SECONDS );
		if ( $days <= 1 )
			$days = 1;
		$since = sprintf( jols_n( '%s day', '%s days', $days ), $days );
	} elseif ( $diff < 30 * DAY_IN_SECONDS && $diff >= WEEK_IN_SECONDS ) {
		$weeks = round( $diff / WEEK_IN_SECONDS );
		if ( $weeks <= 1 )
			$weeks = 1;
		$since = sprintf( jols_n( '%s week', '%s weeks', $weeks ), $weeks );
	} elseif ( $diff < YEAR_IN_SECONDS && $diff >= 30 * DAY_IN_SECONDS ) {
		$months = round( $diff / ( 30 * DAY_IN_SECONDS ) );
		if ( $months <= 1 )
			$months = 1;
		$since = sprintf( jols_n( '%s month', '%s months', $months ), $months );
	} elseif ( $diff >= YEAR_IN_SECONDS ) {
		$years = round( $diff / YEAR_IN_SECONDS );
		if ( $years <= 1 )
			$years = 1;
		$since = sprintf( jols_n( '%s year', '%s years', $years ), $years );
	}

	return $since;
}

function jols_get_currentuserinfo() {
	return false;
}
...
function jols_add_filter($filter, $func) {
  switch ($filter) {
...
function jols_get_categories() {
	return false;
}
function jols_get_tags() {
	return false;
}
function jols_get_taxonomies() {
	return false;
}

function jols_get_allowed_mime_types() {
...
*/
function jols_sanitize_file_name( $filename ) {
...
function jols_check_filetype( $filename, $mimes = null ) {
	if ( empty($mimes) )
		$mimes = jols_get_allowed_mime_types();
	$type = false;
	$ext = false;
...
function jols_sanitize_title($title) {
