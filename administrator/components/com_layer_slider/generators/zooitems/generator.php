...
class OfflajnGenerator_ZooItems extends OfflajnGenerator {
  ...
  function getData() {
    ...
    $order = strlen($orderby) && in_array($orderby, $this->_orderbys) ? $orderby.' '.$ordering.' ' : 'a.name ASC';

		// execute query
		$items = $this->app->table->item->all(compact('select', 'from', 'conditions', 'order', 'limit'));
    // set renderer
    $renderer = $this->app->renderer->create('item')->addPath(array($this->app->path->path('component.site:')));
    ...
    for ($i = 0; $i < count($result); $i++) {
      ...
      $result[$i]['thumbnail'] = isset($result[$i]['image']) && strlen($result[$i]['image'])>0 ? $result[$i]['image'] : (strlen($result[$i]['content_image_url'])>0 ? $result[$i]['content_image_url'] : LS_ROOT_URL . '/static/img/blank.gif');
