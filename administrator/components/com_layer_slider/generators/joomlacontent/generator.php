...
class OfflajnGenerator_JoomlaContent extends OfflajnGenerator {
	...
	function getData() {
		...
		for ($i = 0; $i < count($result); $i++) {
			...
			$result[$i]['thumbnail'] = isset($images['image_intro']) && strlen($images['image_intro'])>0 ? $this->url.$images['image_intro'] : (isset($images['image_fulltext']) && strlen($images['image_fulltext'])>0 ? $this->url.$images['image_fulltext'] : (strlen($result[$i]['content_image_url'])>0 ? $result[$i]['content_image_url'] : LS_ROOT_URL . '/static/img/blank.gif'));
