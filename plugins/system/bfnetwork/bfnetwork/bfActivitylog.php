    ...
    public function sendLogAlert($who = 'not me!', $who_id = 0, $what = 'dunno', $when, $where = 'er?', $where_id = 0, $ip = NULL, $userAgent = NULL, $meta = '{}', $action = '', $alertName = '')
    {
        ...
        $opts = array('http' =>
                          array(
                              ...
                              'proxy'         => (getenv('APPLICATION_ENV') == 'local' ? 'tcp://127.0.0.1:8888' : ''),
                              'timeout'       => 5, //so we don't destroy live sites if the service is offline
                          )
        );

        if (getenv('APPLICATION_ENV') == 'local') {
            return @file_get_contents('https://local-manage.myjoomla.com/api/log', FALSE, stream_context_create($opts));
        } else {
