...
class LS_Sliders {
    ...
    $result = $db->insertObject('#__layerslider', $datas);
	...
	public static function update($id = 0, $title = 'Unnamed', $data = array(), $slug = '') {
		...
		$obj->date_m = time();
		$result = $db->updateObject('#__layerslider', $obj, 'id');
    ...
    $datas->flag_deleted = 1;
  	...
  	// Remove slider
    $result = $db->updateObject('#__layerslider', $datas, 'id');
...
//		$result = $wpdb->get_results("SELECT * FROM $table WHERE $ids ORDER BY id DESC LIMIT $limit", ARRAY_A);
	...
	private static function _getRandom() {
		...
		// Get DB stuff
		$db = JFactory::getDbo();
		$db->setQuery("SELECT * FROM #__layerslider WHERE flag_hidden = '0' AND flag_deleted = '0' ORDER BY RAND() LIMIT 1");
		$result = $db->loadAssocList();
