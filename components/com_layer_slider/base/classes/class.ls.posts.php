...
class LS_Posts {
	...
	public function getParsedObject() {
		...
		foreach($this->posts as $key => $val) {
			...
			$ret[$key]['url'] = get_permalink($val->ID);
			$ret[$key]['date-published'] = date(jols_get_option('date_format'), strtotime($val->post_date));
			$ret[$key]['date-modified'] = date(jols_get_option('date_format'), strtotime($val->post_modified));
			...
			$ret[$key]['title'] = htmlentities(jols__($val->post_title));
			$ret[$key]['content'] = wp_strip_all_tags(jols__($val->post_content));
	...
	public function getWithFormat($str, $textlength = 0) {
		...
		if(stripos($str, '[url]') !== false) {
			$str = str_replace('[url]', get_permalink($this->post->ID), $str);
		}

		// Date published
		if(stripos($str, '[date-published]') !== false) {
			$str = str_replace('[date-published]', date(jols_get_option('date_format'), strtotime($this->post->post_date)), $str); }

		// Date modified
		if(stripos($str, '[date-modified]') !== false) {
			$str = str_replace('%date-modified]', date(jols_get_option('date_format'), strtotime($this->post->post_modified)), $str); }
	...
	public function getTitle() {
		if(is_object($this->post)) { return jols__($this->post->post_title); }
			else { return false; }
	}
	...
	public function getContent($length = false) {
		...
		if(empty($length)) {
			return wp_strip_all_tags(jols__($this->post->post_content));
		} else {
			return substr(wp_strip_all_tags(jols__($this->post->post_content)), 0, $length);
		}
	}
