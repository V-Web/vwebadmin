...
var LS_SublayerPage = 0; // Content tab by default
var LS_SublayerTr = jQuery();
var LS_Sublayer = jQuery();
...
var lsScreenOptionsActions = {
	...
	saveSettings : function(form, reload) {
		...
		// Save settings
		jQuery.post(ajaxsaveurl, jQuery.param({ action : 'ls_save_screen_options', options : options }), function() {
...
var LayerSlider = {
	...
	selectLayer : function(el) {
		...
		// Open new layer & Open sublayer
		jQuery(el).addClass('active');
		jQuery('#ls-layers .ls-layer-box').eq( jQuery(el).index() ).addClass('active').find('.ls-sublayers > tr.active').click();
	...
	duplicateLayer : function(el) {
		...
		// Append new tab
		var tab = jQuery('<a href="#">Slide #0<span class="dashicons dashicons-dismiss"></span></a>').insertBefore('#ls-layer-tabs a:last');
		...
		// generate ls-ids
		// var i, lsid, $lsid = clone.find('.ls-id');
		// for (i = 0; i < $lsid.length - 1; i++) {
		// 	lsid = uniqid('ls');
		// 	clone.find('.connect-hover [value='+ $lsid[i].value +']').attr('value', lsid);
		// 	$lsid[i].value = lsid;
		// }
	...
	addSublayer : function(el) {
		...
		// add ls-id
		// clone.find('.ls-id').val(uniqid('ls'));

		// Update preview
		LayerSlider.generatePreview( jQuery(el).closest('.ls-layer-box').index() );

		// Open it
		LS_SublayerPage = 0; // start with content tab
		clone.click();
		LayerSlider.addColorPicker( clone.find('.ls-colorpicker') );
	},

	selectSubLayer : function(tr, el) {

		if(tr == LS_SublayerTr[0]) {
			return;
		}

		// Open the new one & close other sublayers
		LS_SublayerTr = jQuery(tr).addClass('active');
		LS_SublayerTr.siblings('.active').removeClass('active');

		this.selectSublayerPage( LS_SublayerTr.find('.ls-sublayer-nav')[0].children[LS_SublayerPage] );

		if(!el) {
			el = jQuery('.ls-layer-box.active .draggable')[0].children[ LS_SublayerTr.index() ];
		}
		LS_Sublayer = jQuery(el).addClass('ls-selected');
		LS_Sublayer.siblings('.ls-selected').removeClass('ls-selected');

		// Hide Timeline
		lsTimeLine.hide( LS_SublayerTr.closest('table').find('.ls-tl') );

		// Update responsibility icons
		LayerSlider.updateSubLayerDevices();
	},

	updateSubLayerDevices: function() {
		var fake = jQuery('<i>').addClass( LS_SublayerTr.find('.ls-classes').val() );
		var icons = LS_SublayerTr.find('.ls-devices .dashicons').removeClass('active');
		if (fake.hasClass('ls-hide-phone')) icons.eq(0).addClass('active');
		if (fake.hasClass('ls-hide-tablet')) icons.eq(1).addClass('active');
		if (fake.hasClass('ls-hide-desktop')) icons.eq(2).addClass('active');
	},

	selectSublayerPage : function(el) {
		LS_SublayerPage = jQuery(el).index();
		...
		jQuery(el).addClass('active');
		var $slp = jQuery(el).parent().next().find('.ls-sublayer-page').eq(LS_SublayerPage).addClass('active');

		if (LS_SublayerPage == 1) { // transition tab: show/hide split-text options
			$slp.prev().find('.active[data-section=text]').length ? $slp.find('.splittext').show() : $slp.find('.splittext').hide();
		}
	},
	...
	duplicateSublayer : function(el) {
		...
		// Clone sublayer
		var clone = jQuery(el).closest('tr').clone();
		...
		// add 10px | 1% offset
		var left = clone.find('.ls-left');
		left.val(left.val().indexOf('%') < 0 ? parseInt(left.val()) + 10 : parseFloat(left.val()) + 1 + '%');
		var top = clone.find('.ls-top');
		top.val(top.val().indexOf('%') < 0 ? parseInt(top.val()) + 10 : parseFloat(top.val()) + 1 + '%');
		...
		setTimeout(function() {
			LayerSlider.selectSubLayer(clone[0]);
		}, 1);
	},
	...
	generatePreview : function(index) {
		...
		var post = posts && posts[postOffset];

		// Get background image
		var bgImage = $settings.find('input[name="backgroundimage"]').val();

		// Set backgrounds
		preview.css({
			backgroundImage: bgImage ? 'url('+bgImage+')' : 'none',
			backgroundColor: $settings.find('input[name="backgroundcolor"]').val() || 'transparent',
			backgroundSize: $settings.find('input[name="background_size"]').val() || 'cover',
			backgroundRepeat: $settings.find('select[name="background_repeat"]').val() || 'no-repeat',
			backgroundPosition: $settings.find('input[name="background_position"]').val() || '50% 50%',
			backgroundAttachment: $settings.find('select[name="background_behaviour"]').val() || 'scroll'
		});
		...
		if(background == '[image-url]') {
			background = post ? post['image-url'] : '';
			jQuery('.slide-image:eq(0) img', slide).attr('src', background);
		}

		// Set slide background
		if(background != '') {
			var bgSize = jQuery('select[name=imgsize]', slide).val();
			draggable.css({
				backgroundImage : 'url("'+background+'")',
				backgroundPosition : jQuery('select[name=imgposition]', slide).val(),
		...
		slide.find('.ls-sublayers > tr').each(function(layerkey) {
			...
			if(type == 'img') {
				...
				for(var key in post) {
					if(html.indexOf('['+key+']') !== -1) {
						if( (key == 'title' || key == 'content' || key == 'excerpt' || key == 'introtext' || key == 'fulltext') && textlength > 0) {
			...
			var setPositions = function(){
				...
				if(top === undefined) {
					...
					var topPercent = parseFloat(top);
					if (topPercent == 0)
						item.css({ top : draggable.height() / 100 * topPercent });
					else if (topPercent == 100)
						item.css({ top : draggable.height() / 100 * topPercent - item.outerHeight() });
					else
						item.css({ top : draggable.height() / 100 * topPercent - item.outerHeight() / 2 });
				} else {
				...
				if(left === undefined) {
					...
					var leftPercent = parseFloat(left);
					if (leftPercent == 0)
						item.css({ left : draggable.width() / 100 * leftPercent });
					else if (leftPercent == 100)
						item.css({ left : draggable.width() / 100 * leftPercent - item.outerWidth() });
					else
						item.css({ left : draggable.width() / 100 * leftPercent - item.outerWidth() / 2 });
				} else {
    ...
    if (draggable[0])
      LS_Sublayer = jQuery(draggable[0].children[ LS_SublayerTr.index() ]).addClass('ls-selected');
	...
	openMediaLibrary : function() {
		...
		if(window.newMediaUploader == true) {
	...
	selectDragElement : function(el) {
		this.selectSubLayer(jQuery(el).closest('.ls-layer-box').find('.ls-sublayers')[0].children[ jQuery(el).index() ], el);
	},
	...
	play : function( index ) {
		...
		if(layerslider.children().length > 0) {
			...
			jQuery(window).off('resize.ls scroll.ls');

			// Remove Timeline slider
			lsTimeLine.remove();

			LayerSlider.generatePreview( index );
			LayerSlider.play.running = false;
			return;
		}
		...
		jQuery('#ls-layers .ls-layer-box').each(function(slidekey) {
			...
			// Get post content if any
			var postOffset = jQuery('input[name="post_offset"]', slide).val();
			if(postOffset == -1) { postOffset = slidekey; }
		...
		// Get slider settings
		var form = document.getElementById('ls-slider-form');
		var thumb_nav = jQuery(form.thumb_nav).val();
		...
		jQuery(layerslider).layerSlider({
			...
			firstLayer : (index + 1),
			autoStart : false,
			pauseOnHover : form.pauseonhover.checked,
			// Appearance
			skin : jQuery(form.skin).val() || 'preview',
			skinsPath : pluginPath + 'skins/',
			globalBGColor: form.backgroundcolor.value,
			globalBGImage: form.backgroundimage.value,
			globalBGPosition: form.background_position.value,
			globalBGSize: form.background_size.value,
			globalBGRepeat: jQuery(form.background_repeat).val(),
			globalBGBehaviour: jQuery(form.background_behaviour).val(),
			// Navigation area
			navPrevNext: form.navprevnext.checked,
			navStartStop: form.navstartstop.checked,
			navButtons: form.navbuttons.checked,
			hoverPrevNext: form.hoverprevnext.checked,
			hoverBottomNav: form.hoverbottomnav.checked,
			showBarTimer: form.bartimer.checked,
			showCircleTimer: form.circletimer.checked,
			// Thumbnail navigation (always mode isn't supported on admin)
			thumbnailNavigation: thumb_nav == 'always' ? 'disabled' : thumb_nav,
			// Parallax
			parallaxType: jQuery(form.parallaxtype).val(),
			slideParallax: form.slideparallax.value,
			parallaxOrigin: jQuery(form.parallaxorigin).val(),
			parallaxScrollDuration: form.parallaxscrollduration.value,
			// Videos
			autoPlayVideos: form.autoplayvideos.checked,
			autoPauseSlideshow: jQuery(form.autopauseslideshow).val(),
			youtubePreview: jQuery(form.youtubepreview).val(),
			// Event Callbacks
			cbInit : function(){
				lsTimeLine.create();
				LayerSlider.play.running = false;
	...
	save : function(el) {
		...
		// Slider Settings
		var settings = jQuery('.ls-slider-settings, #ls-post-options').find('input:not(.nochange), select, textarea');
		var sliderSize = jQuery('.ls-slider-size.active');
		if (sliderSize.is('.responsive')) {
			// don't save Responsive under
			settings = settings.filter(':not(#responsive-under)');
		} else if (sliderSize.is('.full-width, .full-screen')) {
			// don't save Max-width
			settings = settings.filter(':not(#max-width)');
		}

		// Save slider
		jQuery.ajax({
			type: 'post', url: ajaxsaveurl, dataType: 'text',
			data: {
			...
			complete : function(data) {
				...
				// autologin at save when session expired
				if (data.responseText.match(/^<!DOCTYPE/i)) {
					return LayerSlider.adminLogin('Please login at the following popup window, to save your slider', true);
				}
	...
	adminLogin: function(msg, save) {
		SqueezeBox.open('', {
			...
			onClose: function() {
				...
				if (save) jQuery('.ls-publish').removeClass('saved').find('button').removeAttr('disabled').click();
			}
		});
	},
...
var LS_PostOptions = {
	...
	init : function() {
		...
		jQuery('#ls-post-options select:not(.ls-post-taxonomy, .post_offset, .generator_type)').change(function() {
		...
		jQuery('#ls-post-options select.generator_type').change(function() {
			LS_PostOptions.changeGenerator(this);
		});
    window.generatorFilters= {};
	},
	...
	getTaxonoies : function(select) {
		...
		if(jQuery(select).val() == 0) {
			...
			jQuery.post(ajaxsaveurl, jQuery.param({ action : 'ls_get_taxonomies', taxonomy : jQuery(select).val() }), function(data) {
	...
	change : function(el) {
		...
		jQuery.post(ajaxgeneratorurl, jQuery.param({ action: 'ls_get_post_details', params : items }), function(data) {
	...
	changeGenerator : function(el) {

		// Get options
		jQuery('#ls-post-options').find('select').each(function() {
			window.generatorFilters[ jQuery(this).data('param') ] = jQuery(this).val();
		});

		jQuery.post(ajaxfilterurl, jQuery.param({ params : window.generatorFilters }), function(data) {
			// Handle data
		  var parsed = jQuery.parseJSON(data);
      var filters = jQuery('#ls-post-options .ls-post-filters').empty();
      filters[0].innerHTML = parsed.filters;
      jQuery('#ls-post-options .ls-post-adv-settings select:first').remove();
      jQuery('#ls-post-options .ls-post-adv-settings .half:first').prepend(parsed.orderby);
      jQuery('.ls-post-placeholders').html(parsed.taglist);
	...
	update : function(el, data) {
		...
		if(data.length === 0) {
			preview.append( jQuery('<li>')
				.append( jQuery('<h4>', { 'text' : 'No items were found with the current filters.' }) )
			);
...
/*		document.location.href.indexOf('page=layerslider') != -1 &&
		...
		// Import sliders
		jQuery('#ls-import-sliders-button').click(function(e) {
			e.preventDefault();
			var $this = $(this);
			...
			$(document.body).css('overflow', 'hidden').on('keyup.lsimport', function(e) {
				if (e.keyCode == 27) $('.ls-add-slider-overlay').click();
			});
			var overlay = $('<div>', {'class': 'ls-overlay ls-add-slider-overlay'}).click(function() {
				$(document.body).css('overflow', '').off('keyup.lsimport');
			}).prependTo('body');
			$('<i class="flaticon-delete ls-import-close">').css('top', top-32).appendTo(overlay);
			...
			if (!popup.children().length) {
				...
				popup.on('click', 'a[data-import]', function onImport() {
					...
					$.getScript(location.protocol+'//offlajn.com/index2.php?option=com_ls_import&task=request&slider='+$(this).data('import'));
				})
				var slideDur = 400;
		...
		// Import
		jQuery('form.ls-import-box').submit(function(e) {
			if (!this.import_file.value) {
				return e.preventDefault();
			}
			$(this).find('button').addClass('saving').text('Importing');
		});
	...
	// Editor view
		...
		jQuery(window).load(function() {
			...
			// on skin change
			var skin = $('select[name=skin]').on('change', function() {
				skin = $(this).val();
				$('#ls-skin').attr('href', pluginPath + 'skins/' + skin + '/skin.css');
				$('#ls-pages').attr('class', 'ls-' + skin);
			}).val();
			// load actual skin
			window.pluginPath && $('<link>').appendTo('head').attr({
				id: 'ls-skin',
				rel: 'stylesheet',
				type: 'text/css',
				href: pluginPath + 'skins/' + skin + '/skin.css'
			});
			$('#ls-pages').attr('class', 'ls-' + skin);
		});
		...
		var $dim = $('.ls-slider-dimensions');
		$dim.on('click', '.ls-slider-size', function(e) {
			var $this = $(this);
			var $width = $('#slider-width');
			var $containerWidth = $('#container-width');
			$this.addClass('active').siblings().removeClass('active');
			if ($this.hasClass('non-responsive')) {
				$dim.find('input[name=responsive]+.on, input[name=forceresponsive]+.on, input[name=fullpage]+.on').click();
				$('#slider-width').closest('tr').show();
				$('#slider-height-lbl').show().next().hide();
				$('#container-width, #max-width, #responsive-under, #reduce-height').closest('tr').hide();
				$width.val().indexOf('%') < 0 || $width.val( $containerWidth.val() ).change();
				$containerWidth.val('').change();
			} else if ($this.hasClass('responsive')) {
				$dim.find('input[name=responsive]+.off, input[name=forceresponsive]+.on, input[name=fullpage]+.on').click();
				$('#slider-width, #max-width').closest('tr').show();
				$('#slider-height-lbl').show().next().hide();
				$('#container-width, #responsive-under, #reduce-height').closest('tr').hide();
				$width.val().indexOf('%') < 0 || $width.val( $containerWidth.val() ).change();
				$containerWidth.val('').change();
			} else if ($this.hasClass('full-width')) {
				$dim.find('input[name=responsive]+.on, input[name=forceresponsive]+.off, input[name=fullpage]+.on').click();
				$('#container-width, #responsive-under').closest('tr').show();
				$('#slider-height-lbl').show().next().hide();
				$('#slider-width, #max-width, #reduce-height').closest('tr').hide();
				parseInt($containerWidth.val()) || $containerWidth.val( $width.val() ).change();
				$width.val('100%').change();
			} else if ($this.hasClass('full-screen')) {
				$dim.find('input[name=responsive]+.on, input[name=forceresponsive]+.off, input[name=fullpage]+.off').click();
				$('#container-width, #container-height, #reduce-height').closest('tr').show();
				$('#slider-height-lbl').hide().next().show();
				$('#slider-width, #slider-height, #max-width, #responsive-under').closest('tr').hide();
				parseInt($containerWidth.val()) || $containerWidth.val( $width.val() ).change();
				$width.val('100%').change();
			}
		}).on('focus blur', 'input', function() {
			var $active = $dim.find('.ls-slider-size.active');
			if (this.name != 'height' && $active.hasClass('full-width') || $active.hasClass('full-screen'))
				$active.toggleClass(this.id);
		});
		...
		jQuery('#ls-layers').on('click', '.ls-url-prompt', function(e){
			e.preventDefault();
			// Image layer
			var $target = jQuery(this).closest('.slide-image, .ls-image-uploader');
			// Slide options
			var url = prompt('Enter an image URL', $target.find('input:eq(1)').val());
		...
		LS_SublayerTr = $('.ls-layer-box.active .ls-sublayers > tr.active').click();


		// Sublayer pages
		jQuery('#ls-layers').on('click', '.ls-sublayer-nav a:not(.duplicate, .remove)', function(e) {
			e.preventDefault();
			LayerSlider.selectSublayerPage(this);
		});

		// Remove sublayer
		jQuery('#ls-layers').on('click', '.ls-sublayer-nav a.remove', function(e) {
			e.preventDefault();
			LayerSlider.removeSublayer(this);
		});

		// Duplicate sublayer
		jQuery('#ls-layers').on('click', '.ls-sublayer-nav a.duplicate', function(e) {
			e.preventDefault();
		...
		// Sublayer: Classes
		jQuery('#ls-layers').on('change', '.ls-classes', function() {
			LayerSlider.updateSubLayerDevices();
		});

		// Sublayer: Attributes
		jQuery('#ls-layers').on('change', '.ls-sublayer-attributes input', function() {
			LayerSlider.willGeneratePreview( jQuery(this).closest('.ls-layer-box').index() );
		});

		// Sublayer: Style
		jQuery('#ls-layers').on('input', '.ls-sublayer-style input, .ls-sublayer-style select, .ls-sublayer-style textarea', function(e) {
			LayerSlider.willGeneratePreview( jQuery(this).closest('.ls-layer-box').index() );
		});
		...
		// Sublayer: HTML
		jQuery('#ls-layers').on('input', '.ls-sublayers textarea[name="html"]', function() {
			LayerSlider.willGeneratePreview( jQuery(this).closest('.ls-layer-box').index() );
		});

		// Post content text length
		jQuery('#ls-layers').on('input', '.ls-sublayers input[name="post_text_length"]', function() {
			LayerSlider.willGeneratePreview( jQuery(this).closest('.ls-layer-box').index() );
		});
		...
		jQuery('#ls-layers').on('click', '.ls-preview-button', function(e) {
			e.preventDefault();
			if (!LayerSlider.play.running) {
				LayerSlider.play.running = true;
				LayerSlider.play( jQuery(this).closest('.ls-layer-box').index() );
			}
		});

		// Preview drag element select
		jQuery('#ls-layers').on('mousedown', '.draggable > *', function() {
			$(document.activeElement).is(':input') && document.activeElement.blur();
		...
		jQuery('#ls-slider-form').submit(function(e) {
			e.preventDefault();
			if (LayerSlider.timeout) {
				LayerSlider.timeout = clearTimeout(LayerSlider.timeout);
		...
		jQuery('#ls-layers').on('mousedown', '.ls-sublayer-sortable-handle', function(){
			...
			jQuery(document).one('mouseup', function(){
				jQuery('.ls-layer-box.active .ls-sublayer-sortable').removeClass('dragging');
			});
		});
		...
		// Use post URL
		jQuery('#ls-layers').on('click', '.ls-post-url', function(e) {
			e.preventDefault();
			jQuery(this).closest('.ls-slide-link, .slide-link').find('input').val('[url]');
		});

		// Use post image as slide background
		jQuery('#ls-layers').on('click', '.slide-image .ls-post-image', function(e) {
			e.preventDefault();
			jQuery(this).closest('.slide-image').find('input[name="imageId"], input[name="backgroundId"]').val('');
			jQuery(this).closest('.slide-image').find('input[name="image"], input[name="background"]').val('[image-url]');
	...
	// handle hide on phone|mobile|desktop
	jQuery('#ls-layers').on('click', '.ls-devices .dashicons', function() {
		var hide = 'ls-hide-' + this.className.match(/phone|tablet|desktop/)[0];
		var $classes = $(this).closest('.ls-sublayer-wrapper').find('.ls-classes');
		var classes = $('<i>').addClass( $classes.val() ).toggleClass(hide).attr('class');
		$classes.val(classes);
	...
	jQuery('#ls-layers').on('click', '.ls-preview-mode .dashicons', function() {
		var mode = 'ls-' + this.className.match(/phone|tablet|desktop/)[0];
		$('#ls-layers').removeClass('ls-phone ls-tablet ls-desktop').addClass(mode);

		LayerSlider.generatePreview( jQuery(this).closest('.ls-layer-box').index() );
	});

	// handle horizontal move icons
	$('#ls-pages').on('click', '.ls-move-horz', function() {
		var x, move = $(this).data('move');
		x = move == 'center' ? '50%' : (move == 'left' ? '0%' : '100%');
		LS_SublayerTr.find('.ls-left').val(x);
		LS_Sublayer.css('left', (LS_Sublayer.parent().width() - LS_Sublayer.outerWidth()) * parseInt(x) / 100);
	// handle vertical move icons
	}).on('click', '.ls-move-vert', function() {
		var y, move = $(this).data('move');
		y = move == 'middle' ? '50%' : (move == 'top' ? '0%' : '100%');
		LS_SublayerTr.find('.ls-top').val(y);
		LS_Sublayer.css('top', (LS_Sublayer.parent().height() - LS_Sublayer.outerHeight()) * parseInt(y) / 100);
	// update slide image settings
	}).on('change', 'select[name=imgsize], select[name=imgposition]', function() {
		LayerSlider.generatePreview( jQuery(this).closest('.ls-layer-box').index() );
	});

	// handle layer positioning, play/stop preview for space
	$(document).on('keydown', function(e) {
		if ($(e.target).is('textarea')) return;
		if ($(e.target).is('input')) {
			if (e.target.form.id != 'ls-add-slider-template' && e.keyCode == 13) { // enter
				e.target.blur();
				e.preventDefault();
			}
			return;
		...
		if (e.keyCode == 32) { // space
			if (!LayerSlider.play.running) {
				LayerSlider.play.running = true;
				LayerSlider.play( jQuery('.ls-layer-box.active').index() );
			}
			return e.preventDefault();
		}
		if (LS_Sublayer.hasClass('disabled') || LS_Sublayer.css('display') == 'none') return;
		var i = 0, j = 0;
		switch (e.keyCode) {
			case 67: // c
				if (e.ctrlKey || e.metaKey) {
					var selection = window.getSelection ? getSelection().toString() : document.selection.createRange().htmlText;
					if (selection) return; // disable layer copy when any text is selected
					if (e.shiftKey) {
						LS_SublayerTr.find('.copy[data-storage=lsStyle]').click();
					} else {
						var obj = LS_SublayerTr.inputsToObj();
						obj.section = LS_SublayerTr.find('.ls-layer-kind .active').data('section');
						obj.left = obj.left.indexOf('%') < 0 ? parseInt(obj.left)+10 : parseFloat(obj.left)+1+'%';
						obj.top = obj.top.indexOf('%') < 0 ? parseInt(obj.top)+10 : parseFloat(obj.top)+1+'%';
						localStorage.lsLayer = JSON.stringify(obj);
					}
					return e.preventDefault();
				}
			case 86: // v
				if (e.ctrlKey || e.metaKey) {
					if (e.shiftKey) {
						LS_SublayerTr.find('.paste[data-storage=lsStyle]').click();
					} else if (localStorage.lsLayer) {
						var obj = JSON.parse(localStorage.lsLayer);
						$('.ls-layer-box.active .ls-add-sublayer').click();
						LS_SublayerTr.find('.ls-layer-kind li[data-section='+ obj.section +']').click();
						LS_SublayerTr.find('.ls-image img').attr('src', obj.image);
						LS_SublayerTr.objToInputs(obj);
					}
					return e.preventDefault();
				}
			case 68: if (e.ctrlKey || e.metaKey) // ctrl/cmd + d
				return LayerSlider.duplicateSublayer(LS_SublayerTr[0]), e.preventDefault();
			case 08: // backspace
			case 46: // delete
				return LayerSlider.removeSublayer(LS_SublayerTr[0]), e.preventDefault();
		...
		if (i) {
			var x = parseInt(LS_Sublayer.css('left'));
			x += e.shiftKey ? i * 10 : i;
			LS_Sublayer.css('left', x);
			LS_SublayerTr.find('.ls-left').val(x + 'px');
			return e.preventDefault();
		}
		if (j) {
			var y = parseInt(LS_Sublayer.css('top'));
			y += e.shiftKey? j*10 : j;
			LS_Sublayer.css('top', y);
			LS_SublayerTr.find('.ls-top').val(y + 'px');
			return e.preventDefault();
		}
	});
